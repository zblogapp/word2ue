...
// 这里写过文档解析的过滤规则
    ...
    UE.registerUI('editword', function (editor, uiName) {
        ...
        editor.addListener('ready', function () {
            ...
            $(btn.getDom()).width(0).height(0).hide();
        });
        ...
        function initWordBtn(containerBtn) {
            ...
            UE.dom.domUtils.on(btnIframe, 'load', function () {
                ...
                '<script src="docparser/webuploader/webuploader.flashonly.js"></script>' +
                '<link rel="stylesheet" type="text/css" href="docparser/webuploader/webuploader.css">' +
                '</head>' +
                '<body>' +
            ...
            */
            ...
            me.on('uploader_filesQueued', function (type, files) {
                if (files && files[0]) {
                    ...
                    editor_api.editor.content.obj.trigger('showmessage', {
                        'id': files[0].id,
                        'type': 'info',
                        'content': msgContent,
                        'keepshow': true
            ...
            me.on('uploader_uploadSuccess', function (type, file, r) {
                ...
                if (bdjson['document.xml'] && bdjson['document.xml'].length && bdjson['status'] && bdjson['status']['success']) {
                    editor_api.editor.content.obj.trigger('updatemessage', fileId, {
                        'type': 'success',
                        'content': '解析成功!',
                        'timeout': 2000
                    });
                    editor_api.editor.content.obj.setWkContent(bdjson['document.xml']);
                } else {
                    showErrorMessage(file);
                }
            ...
            function showErrorMessage(file){
                editor_api.editor.content.obj.trigger('updatemessage', file.id, {
                    'type': 'error',
